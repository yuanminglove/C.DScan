...
var HostGroupDao = {
	...
	"save" : function(hg,cb){
		...
		if(hg.groupId!=""){
			HostGroupModel.find({"_id":hg.groupId},function(err,data){
				if(err){
					...
					HostGroupModel.update({"_id":data._id},data,function(err){
						if(err){
	...
	"deleteById" : function(id,cb){
		HostGroupModel.findOne({"_id":id},function(err,data){
			if(err || !data){
				...
				console.log(data)
				HostGroupModel.remove({"_id":data._id},function(err){
					if(err){
	...
	"addToGroup" : function(id,page,cb){
		...
		HostGroupModel.findOne({"_id":id},function(err,hostGroup){
			if(err || !hostGroup){
				...
				// cutils.log(hostIds)
				HostModel.update({"_id":{"$in":hostIds}},{"groupId":hostGroup._id},{ "multi": true },function(err,data){
					if(err){
	...
	"findGroupAndHosts" : function(id,cb){
		HostGroupModel.findOne({"_id":id},function(err,hostGroup){
			if(err || !hostGroup){
				...
				HostModel.find({"groupId":hostGroup._id},function(err,data){
					if(err){
						console.log("[HostGroupDao ERROR : ] -findGroupAndHosts- Host find error ");
						console.log(err);
						cb(true);
